...
bool SBUS::readCal(float* calChannels, bool* failsafe, bool* lostFrame)
{
	...
	if(read(&channels[0],failsafe,lostFrame)) {
		// linear calibration
		for (uint8_t i = 0; i < _numChannels; i++) {
			calChannels[i] = channels[i] * _sbusScale[i] + _sbusBias[i]; //Why is biasing done to the all input signal, and its not trim?
			if (_useReadCoeff[i]) {
...
void SBUS::write(uint16_t* channels)
{
	...
	/* assemble the SBUS packet - Reconstructing the SBUS package in the reverse way*/
	// SBUS header
